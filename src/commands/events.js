    ...
    try {
      ...
      if (isPrivate) {
        ...
        if (commandName === "eventos agendados") {
          ...
          return privateContext.replyText(eventService.formatEventList(manageable.list, "📅 EVENTOS AGENDADOS"));
        }
        ...
        if (["ver evento", "evento ver"].includes(commandName)) {
          if (!args[0]) return privateContext.replyText("❌ Use: !ver evento E0001");
        ...
        if (["historico eventos", "eventos historico"].includes(commandName)) {
          const manageable = await guidedFlow.manageableEvents(client, privateContext, privateContext.role, ["finished", "cancelled", "archived"]);
